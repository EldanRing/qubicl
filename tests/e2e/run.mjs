...
try {
  ...
  const backgroundAcquire = await rawCall('acquire_lease', { durationSeconds: 60 });
  assert.equal(backgroundAcquire.response.ok, true);
  const backgroundLease = backgroundAcquire.value;
  const backgroundTask = await call('exec_command', { lease: backgroundLease, command: 'printf background-task-ready', yieldTimeMs: 1_000 });
  assert.equal(backgroundTask.output, 'background-task-ready');
  const humanBlockedInput = await rawCall('control_computer', { lease: backgroundLease, action: { type: 'wait', durationMs: 1 } });
  assert.equal(humanBlockedInput.response.status, 409);
  assert.equal(humanBlockedInput.value.error.code, 'human_control_active');
  await waitFor(async () => exec('docker', ['exec', computerExecutorRuntime(computer), 'pgrep', '-u', `${process.getuid()}`, '-x', 'sleep']).then(() => false, () => true), 10_000);
  ...
  const backgroundBlockedInput = await rawCall('control_computer', { lease: backgroundLease, action: { type: 'wait', durationMs: 1 } });
  assert.equal(backgroundBlockedInput.response.status, 409);
  assert.equal(backgroundBlockedInput.value.error.code, 'background_lease');
  await call('release_lease', { lease: backgroundLease });
