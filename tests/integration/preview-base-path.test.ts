...
test('published HTTP previews support path-relative assets and redirects without forwarding operator credentials or cookies', async (context) => {
  const requests: string[] = [];
  const target = createServer((request, response) => {
    requests.push(request.url!);
    assert.equal(request.headers.authorization, undefined);
    assert.equal(request.headers.cookie, 'app-client=present');
  ...
  const headers = {
    ...
    cookie: 'app-client=present; qubicl_view=operator-secret; __Host-qubicl_dashboard=operator-secret',
    'x-qubicl-internal-key': 'internal',
  };
  const redirect = await fetch(`${base}start`, { headers, redirect: 'manual' });
  assert.equal(redirect.status, 302);
  assert.match(redirect.headers.get('set-cookie') ?? '', /^app-session=private(?:;|$)/u);
