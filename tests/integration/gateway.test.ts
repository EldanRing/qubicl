...
import { hashToken, presetDefaults, previewHostname } from '@qubicl/core';
...
function route(id: string, name: string, controlPort: number, token: string, internalKey: string, viewPort = controlPort, controlViewPort = controlPort) {
  return {
    id,
    name,
    host: '127.0.0.1',
...
test('viewer tickets are read-only and token-bound', async (context) => {
  ...
  context.after(() => backend.close());
  const backendPort = (backend.address() as { port: number }).port;
  const controllingBackend = createNetServer((socket) => {
  ...
  const writeRoutes = async (token: string): Promise<void> => {
    await writeFile(routesPath, JSON.stringify({ version: 2, generatedAt: new Date().toISOString(), routes: [route(id, 'viewer', backendPort, token, 'internal-key-that-is-definitely-long-enough', backendPort, controllingPort)] }));
  };
  await writeRoutes('first');
  ...
  const firstObserver = await openWebSocket(port, id, cookie);
