...
This page describes the 0.6 architecture. The
[design decision](decisions/0002-v0.6-capabilities-and-constraints.md) records
the ownership, task, browser, preview, network, recovery, and release-evidence
choices behind it.
...
Each computer image embeds a canonical version-1 manifest and matching OCI labels. The control service uses it to register the exact MCP/OpenAPI tool set. The host stores its expected digest; a mismatch prevents health from succeeding. Control protocol version 11 uses one bounded container per computer while adding resource-specific interactive ownership, retained tasks and services, bounded PTYs, scoped client identity, installed-application discovery, anonymous public extraction, and app-lifetime previews to the existing compact/paginated, skill, browser, desktop, web, and Open Terminal contracts.
...
Open WebUI may instead use `/open-terminal/`. That compatibility namespace exposes an OpenAPI document without caller-supplied lease fields and a bounded native file API rooted in `/home/qubicl`, including metadata, filename/content search, display handoff, filesystem-backed chat uploads, descriptor-streamed ZIP downloads, retained process execution, and paginated output. Qubicl's projected model tools separately include bounded interactive PTY tasks, while Open WebUI's native terminal-emulator and notebook feature flags remain false. Native and generic file operations share the same descriptor-anchored, no-follow traversal and atomic mutation boundary, so validation is not separated from use by a pathname race. Seamless active-file display sends parser-normalized, self-contained static HTML/SVG through Open WebUI's existing file proxy with a response-enforced CSP sandbox; it rejects an initially selected final symlink and embeds only bounded passive assets confined to the named directory. This path does not require direct browser access to a Qubicl preview hostname. Scripted HTML adds an explicit trusted-interactive button to the exact file snapshot already present in that authenticated response. The button expires after five minutes and replaces the nested static document in place, avoiding a second Open WebUI request or browser-visible credential. The interactive document retains a unique-origin sandbox while permitting scripts and disclosed operator-browser networking only after the click. Its Open WebUI port projection lists and proxies only currently listening ports that were explicitly published through Qubicl.

The `browser`, `computer`, and `workstation` contracts expose navigation, accessibility snapshots, ref-based interactions, history/wait, a normal persistent profile, durable downloads and home-confined uploads, dialog and permission controls, explicit viewport geometry, bounded screenshots, and coordinate actions. Qubicl drives the image's existing Chromium rather than downloading another browser. Browser operations are serialized. Stable tab IDs reconcile with human focus, the visible 24-tab agent-open budget reports capacity instead of evicting old tabs, and resource pressure returns an explicit error. Snapshot refs retain exact DOM nodes, survive sibling insertion or reordering, and reject detached or replaced targets; new snapshots and page/tab changes invalidate prior refs. Retained handles are disposed when invalidated or omitted from bounded output. Public rendered extraction uses a separate ephemeral anonymous context rather than the logged-in profile.
...
The gateway authenticates the default compatibility bearer or one named client credential, attaches its ID/label/scopes to the internal request, and reaches the controller on that computer's private network. Interactive leases carry an unguessable ID, fencing generation, and server epoch. Expiry, preemption, or control restart invalidates stale proofs and terminates session-scoped process groups and terminals; retained tasks and services have independent limits and explicit stop controls. Human ownership follows the controlling viewer WebSocket: a visible configurable grace period (10 seconds by default) permits reconnects before release. Because all computer functions share one PID namespace, this is a cooperative managed-process fence: an intentionally evasive process may survive until the computer container is restarted.
...
`computer` and `workstation` contracts additionally expose
`open_desktop_application`, `list_desktop_applications`, and
`close_desktop_application`. Discovery returns a bounded catalog of safe
built-ins and user/system desktop entries whose executable resolves inside an
approved computer bin directory. Launch remains direct without a shell, uses a
sanitized environment, confines document paths below `/home/qubicl`, reserves
bounded capacity before preparation, and records executable identity and
lifecycle. LibreOffice uses a correctly owned isolated profile. Applications
survive human takeover; close requires explicit unsaved-work acknowledgement.
During human control, observation and background work remain available while
interactive input stays fenced.

Coding-oriented tools bound model-facing output. Text reads default to 2,000 lines and 24 KB with explicit continuation offsets; supported image reads and screenshots become native MCP image content. Commands return one combined 24 KB tail by default and provide paginated output plus an explicit save-to-home operation. Retained task output is capped at 100 MB per task, aggregate records are compacted, and completed records/logs default to seven-day retention. Directory listings are deterministic, root-relative, and cursor-paginated. Writes and exact-match multi-edits serialize per path; edits preserve UTF-8 BOMs and line endings and reject ambiguous or overlapping replacements.

## State format 5

```text
~/.qubicl/                       mode 0700
├── config.yaml                  state v5; mode 0600
├── secrets.yaml                 client bearers/internal credentials; mode 0600
