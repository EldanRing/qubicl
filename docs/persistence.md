...
`--stopped` requires the source computer to be stopped. Optional `--encrypt --passphrase-file FILE` uses a local passphrase without persisting it in Qubicl state. Archives have checksummed manifests, safe extraction, explicit retention, and contain only the durable home—not runtime roots or credentials. Migration snapshots are not backups of computer homes, and secret-free `export` intentionally omits credentials and file contents. A separate private backup of the complete state root is still required for disaster recovery of computer identities, tokens, policies, SSH keys, and trash.
